...
export default function AdminWithdrawalRequestsPage() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const response = await fetch(
        `/api/earnings/admin/withdrawal-requests?status=${filter}`
      );
      if (response.ok) {
        const data = await response.json();
        setRequests(data.requests || []);
  ...
  const handleAction = async (
    ...
    try {
      const response = await fetch("/api/earnings/admin/withdrawal-requests", {
        ...
        body: JSON.stringify({ requestId, action, notes: notes || undefined }),
      });
  ...
  // If user is not admin, render nothing (redirect handled)
