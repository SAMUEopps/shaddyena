...
export default function DeliveryHistoryPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      case 'CONFIRMED': return <CheckCircle className="w-3 h-3 xs:w-4 xs:h-4" />;
      case 'DELIVERED': return <Package className="w-3 h-3 xs:w-4 xs:h-4" />;
      case 'CANCELLED': return <XCircle className="w-3 h-3 xs:w-4 xs:h-4" />;
      case 'FAILED': return <AlertCircle className="w-3 h-3 xs:w-4 xs:h-4" />;
      default: return <Clock className="w-3 h-3 xs:w-4 xs:h-4" />;
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center justify-center p-2 xs:p-2.5 sm:p-3 bg-[var(--color-primary)]/10 rounded-xl xs:rounded-2xl mb-3 xs:mb-4 sm:mb-6 animate-bounce-subtle">
              <Clock className="w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 text-[var(--color-primary)]" />
            </div>
            ...
            <div className="flex flex-wrap gap-2 xs:gap-3 sm:gap-4 justify-center">
              <div className="flex items-center space-x-1.5 xs:space-x-2 bg-[var(--color-surface)] px-2 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 rounded-full border border-[var(--color-border)]">
                <Package className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-4 sm:h-4 text-[var(--color-primary)]" />
                <span className="text-[10px] xs:text-xs sm:text-sm">{totalDeliveries} Total Deliveries</span>
              </div>
              <div className="flex items-center space-x-1.5 xs:space-x-2 bg-[var(--color-surface)] px-2 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 rounded-full border border-[var(--color-border)]">
                <DollarSign className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-4 sm:h-4 text-[var(--color-primary)]" />
                <span className="text-[10px] xs:text-xs sm:text-sm">{formatCurrency(totalEarnings)} Total Earned</span>
              </div>
              <button
                onClick={fetchDeliveryHistory}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 xs:py-8 sm:py-10 md:py-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 xs:gap-3 sm:gap-4 md:gap-6 mb-6 xs:mb-8 sm:mb-10 md:mb-12">
          <div className="group bg-[var(--color-surface)] rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:shadow-xl transition-all duration-300">
            ...
            <p className="text-lg xs:text-xl sm:text-2xl font-bold text-[var(--color-text)] mb-0.5 xs:mb-1">
              {deliveries.reduce((count, d) => 
                count + d.suborders.filter(s => s.status === 'DELIVERED' || s.status === 'CONFIRMED').length, 0
              )}
            </p>
            <p className="text-[10px] xs:text-xs sm:text-sm text-[var(--color-text-muted)]">Successful Deliveries</p>
          </div>

          <div className="group bg-[var(--color-surface)] rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:shadow-xl transition-all duration-300">
            <div className="flex items-center justify-between mb-2 xs:mb-3 sm:mb-4">
              <div className="p-1.5 xs:p-2 sm:p-3 bg-red-500/10 rounded-lg xs:rounded-xl group-hover:scale-110 transition-transform">
            ...
            <p className="text-[10px] xs:text-xs sm:text-sm text-[var(--color-text-muted)]">Cancelled/Failed</p>
          </div>
          ...
          <div className="group bg-[var(--color-surface)] rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:shadow-xl transition-all duration-300">
            ...
            <p className="text-[10px] xs:text-xs sm:text-sm text-[var(--color-text-muted)]">Total Earnings</p>
          </div>

          <div className="group bg-[var(--color-surface)] rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:shadow-xl transition-all duration-300">
            <div className="flex items-center justify-between mb-2 xs:mb-3 sm:mb-4">
              <div className="p-1.5 xs:p-2 sm:p-3 bg-blue-500/10 rounded-lg xs:rounded-xl group-hover:scale-110 transition-transform">
                <Award className="w-4 h-4 xs:w-5 xs:h-5 sm:w-6 sm:h-6 text-blue-500" />
              </div>
            </div>
            <p className="text-lg xs:text-xl sm:text-2xl font-bold text-[var(--color-text)] mb-0.5 xs:mb-1">
              {deliveries.length > 0 ? Math.round(totalEarnings / deliveries.length) : 0}
            </p>
            <p className="text-[10px] xs:text-xs sm:text-sm text-[var(--color-text-muted)]">Avg. Earnings/Delivery</p>
          </div>
        </div>

        {/* Filters Section */}
        <div className="bg-[var(--color-surface)] rounded-xl xs:rounded-2xl border border-[var(--color-border)] p-3 xs:p-4 sm:p-6 mb-6 xs:mb-8">
          <div className="flex flex-col sm:flex-row gap-2 xs:gap-3 sm:gap-4">
            {/* Search */}
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 xs:left-4 top-1/2 transform -translate-y-1/2 w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-text-muted)]" />
                <input
                  type="text"
                  placeholder="Search by Order ID..."
                  value={searchInput}
                  onChange={(e) => setSearchInput(e.target.value)}
                  onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
                  className="w-full pl-9 xs:pl-12 pr-3 xs:pr-4 py-2 xs:py-3 text-sm xs:text-base bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg xs:rounded-xl text-[var(--color-text)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/50"
                />
              </div>
            </div>
            ...
            <div className="relative">
              <button
                onClick={() => setShowDatePicker(!showDatePicker)}
                className="flex items-center space-x-1.5 xs:space-x-2 px-3 xs:px-4 py-2 xs:py-3 bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg xs:rounded-xl hover:border-[var(--color-primary)] transition-all w-full sm:w-auto"
              >
                <Calendar className="w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-text-muted)]" />
                <span className="text-sm xs:text-base text-[var(--color-text)]">
                  {dateFilter === 'today' ? 'Today' : 
                   dateFilter === 'week' ? 'This Week' : 
                   dateFilter === 'month' ? 'This Month' : 'All Time'}
                </span>
                <ChevronDown className="w-3 h-3 xs:w-4 xs:h-4 text-[var(--color-text-muted)]" />
              </button>
              
              {showDatePicker && (
                <div className="absolute top-full mt-2 right-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-lg z-10 p-3 xs:p-4 min-w-[180px] xs:min-w-[200px]">
                  <button
                    onClick={() => handleDateFilterChange('today')}
                    className="w-full text-left px-3 xs:px-4 py-1.5 xs:py-2 text-sm hover:bg-[var(--color-background-soft)] rounded-lg transition-colors"
                  >
                    Today
                  </button>
                  <button
                  ...
                  <div className="border-t border-[var(--color-border)] my-2 pt-2">
                    <div className="space-y-2">
                      <input
                        ...
                        value={startDate}
                        onChange={(e) => setStartDate(e.target.value)}
                        className="w-full px-2 xs:px-3 py-1.5 xs:py-2 bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg text-sm"
                      />
                      <input
                        type="date"
                        value={endDate}
                      ...
                      <button
                        onClick={handleCustomDateRange}
                        className="w-full px-3 xs:px-4 py-1.5 xs:py-2 bg-[var(--color-primary)] text-white rounded-lg text-sm hover:bg-[var(--color-primary-hover)]"
                      >
                        Apply Range
                      </button>
                    </div>
            ...
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}
              className="px-3 xs:px-4 py-2 xs:py-3 bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg xs:rounded-xl text-sm xs:text-base text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/50"
            >
              <option value="all">All Status</option>
              <option value="DELIVERED">Delivered</option>
              <option value="CONFIRMED">Confirmed</option>
            ...
            <button
              onClick={exportToCSV}
              className="flex items-center justify-center space-x-1.5 xs:space-x-2 px-4 xs:px-6 py-2 xs:py-3 bg-[var(--color-primary)] text-white rounded-lg xs:rounded-xl hover:bg-[var(--color-primary-hover)] transition-all duration-300 hover:scale-105"
            >
              <Download className="w-4 h-4 xs:w-5 xs:h-5" />
              <span className="text-sm xs:text-base">Export</span>
            </button>
          </div>
          ...
          {(searchQuery || dateFilter !== 'all' || statusFilter !== 'all') && (
            <div className="flex flex-wrap gap-1.5 xs:gap-2 mt-3 xs:mt-4">
              {searchQuery && (
                <span className="inline-flex items-center space-x-1 xs:space-x-2 px-2 xs:px-3 py-0.5 xs:py-1 bg-[var(--color-primary)]/10 rounded-full text-[10px] xs:text-xs sm:text-sm">
                  <span>Search: {searchQuery}</span>
                  <button onClick={handleClearSearch} className="hover:text-[var(--color-primary)]">×</button>
                </span>
              )}
        ...
        {filteredDeliveries.length === 0 ? (
          <div className="bg-[var(--color-surface)] rounded-xl xs:rounded-2xl p-6 xs:p-8 sm:p-12 text-center border border-[var(--color-border)]">
            <div className="inline-flex p-3 xs:p-4 bg-[var(--color-primary)]/10 rounded-full mb-3 xs:mb-4">
              <Package className="w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 text-[var(--color-primary)]" />
            </div>
            <h3 className="text-base xs:text-lg sm:text-xl font-semibold text-[var(--color-text)] mb-1 xs:mb-2">No Delivery History</h3>
            <p className="text-xs xs:text-sm text-[var(--color-text-muted)] mb-3 xs:mb-4">
              You haven't completed any deliveries yet.
            </p>
            <Link
              href="/delivery/assignments"
              className="inline-flex items-center space-x-1.5 xs:space-x-2 px-4 xs:px-6 py-2 xs:py-3 bg-[var(--color-primary)] text-white rounded-lg xs:rounded-xl text-sm xs:text-base hover:bg-[var(--color-primary-hover)] transition-all duration-300"
            >
              <Truck className="w-4 h-4 xs:w-5 xs:h-5" />
              <span>View Active Deliveries</span>
            </Link>
          </div>
        ) : (
          <div className="space-y-2 xs:space-y-3 sm:space-y-4">
            {filteredDeliveries.map((delivery) => (
              <div
                key={delivery._id}
                className="bg-[var(--color-surface)] rounded-xl xs:rounded-2xl border border-[var(--color-border)] hover:border-[var(--color-primary)]/50 transition-all duration-300 overflow-hidden"
              >
                {/* Header */}
                <div
                  onClick={() => toggleExpand(delivery._id)}
                  className="p-3 xs:p-4 sm:p-6 cursor-pointer hover:bg-[var(--color-background-soft)] transition-colors"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 xs:gap-4">
                    <div className="flex items-center space-x-2 xs:space-x-3 sm:space-x-4">
                      ...
                      <div>
                        <p className="text-sm xs:text-base font-semibold text-[var(--color-text)]">Order #{delivery.orderId}</p>
                        <div className="flex items-center space-x-1 xs:space-x-2 mt-0.5 xs:mt-1">
                          <Calendar className="w-2.5 h-2.5 xs:w-3 xs:h-3 text-[var(--color-text-muted)]" />
                          <span className="text-[10px] xs:text-xs text-[var(--color-text-muted)]">{formatDate(delivery.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 xs:gap-3 sm:gap-4">
                      <div className="flex items-center space-x-1 xs:space-x-2">
                        <DollarSign className="w-3 h-3 xs:w-4 xs:h-4 text-green-500" />
                        <span className="text-sm xs:text-base font-semibold text-[var(--color-text)]">
                          {formatCurrency(delivery.suborders.reduce((sum, s) => sum + s.deliveryFee, 0))}
                        </span>
                      </div>
                      <div className={`px-2 xs:px-3 py-0.5 xs:py-1 rounded-full text-[10px] xs:text-xs font-medium flex items-center space-x-1 ${getStatusColor(delivery.suborders[0]?.status)}`}>
                        {getStatusIcon(delivery.suborders[0]?.status)}
                        <span>{getStatusText(delivery.suborders[0]?.status)}</span>
                      </div>
                      {expandedDelivery === delivery._id ? (
                        <ChevronUp className="w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-text-muted)]" />
                      ) : (
                        <ChevronDown className="w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-text-muted)]" />
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Expanded Details */}
                {expandedDelivery === delivery._id && (
                  <div className="border-t border-[var(--color-border)] p-3 xs:p-4 sm:p-6 bg-[var(--color-background-soft)]/30 animate-slide-in">
                    {delivery.suborders.map((suborder, idx) => {
                      const vendor = vendorInfo[suborder.vendorId];
                      return (
                        <div key={suborder._id} className="space-y-3 xs:space-y-4">
                          {/* Vendor Info */}
                          <div className="bg-[var(--color-surface)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                            <div className="flex flex-col xs:flex-row xs:items-start justify-between gap-2 xs:gap-3">
                              <div className="flex items-center space-x-2 xs:space-x-3">
                                <Store className="w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-primary)]" />
                                <div>
                                  <p className="text-sm xs:text-base font-medium text-[var(--color-text)]">{vendor?.businessName || 'Vendor Store'}</p>
                                  <p className="text-[10px] xs:text-xs text-[var(--color-text-muted)]">{vendor?.address}</p>
                                </div>
                              </div>
                              {vendor?.phone && (
                                <button
                                  onClick={() => window.open(`tel:${vendor.phone}`)}
                                  className="flex items-center space-x-1 text-[10px] xs:text-xs text-[var(--color-primary)] hover:underline"
                                >
                                  <Phone className="w-2.5 h-2.5 xs:w-3 xs:h-3" />
                                  <span>Call</span>
                                </button>
                              )}
                            </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 xs:gap-3 sm:gap-4">
                            <div className="bg-[var(--color-surface)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                              <div className="flex items-start space-x-2 xs:space-x-3">
                                <MapPin className="w-3 h-3 xs:w-4 xs:h-4 text-blue-500 mt-0.5" />
                                <div>
                                  <p className="text-[10px] xs:text-xs font-medium text-[var(--color-text)]">Pickup Location</p>
                            ...
                            <div className="bg-[var(--color-surface)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                              <div className="flex items-start space-x-2 xs:space-x-3">
                                <MapPin className="w-3 h-3 xs:w-4 xs:h-4 text-red-500 mt-0.5" />
                                <div>
                                  <p className="text-[10px] xs:text-xs font-medium text-[var(--color-text)]">Dropoff Location</p>
                                  <p className="text-[10px] xs:text-xs text-[var(--color-text-muted)] mt-0.5 xs:mt-1">
                                    {suborder.deliveryDetails?.dropoffAddress || delivery.shipping.address}
                                  </p>
                                  <p className="text-[8px] xs:text-[10px] text-[var(--color-text-muted)] mt-0.5">
                                    {delivery.shipping.city}, {delivery.shipping.country}
                                  </p>
                                </div>
                              </div>
                          ...
                          {/* Order Items */}
                          <div className="bg-[var(--color-surface)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                            <p className="text-xs xs:text-sm font-medium text-[var(--color-text)] mb-2 xs:mb-3">Order Items</p>
                            <div className="space-y-2">
                              {suborder.items.map((item, itemIdx) => (
                                <div key={itemIdx} className="flex justify-between items-center py-1.5 xs:py-2 border-b border-[var(--color-border)] last:border-0">
                                  <div className="flex items-center space-x-2 xs:space-x-3">
                                    {item.image ? (
                                      <img src={item.image} alt={item.name} className="w-8 h-8 xs:w-10 xs:h-10 rounded-lg object-cover" />
                                    ) : (
                                      <div className="w-8 h-8 xs:w-10 xs:h-10 bg-[var(--color-primary)]/10 rounded-lg flex items-center justify-center">
                                        <Package className="w-4 h-4 xs:w-5 xs:h-5 text-[var(--color-primary)]" />
                                      </div>
                                    )}
                                    <div>
                                      <p className="text-[10px] xs:text-xs text-[var(--color-text)]">{item.name}</p>
                                      <p className="text-[8px] xs:text-[10px] text-[var(--color-text-muted)]">Qty: {item.quantity}</p>
                                    </div>
                                  </div>
                                  <span className="text-[10px] xs:text-xs font-medium text-[var(--color-text)]">
                                    {formatCurrency(item.price * item.quantity)}
                                  </span>
                                </div>
                              ))}
                            </div>
                            <div className="mt-2 xs:mt-3 pt-2 xs:pt-3 border-t border-[var(--color-border)] flex justify-between">
                              <span className="text-[10px] xs:text-xs font-medium text-[var(--color-text)]">Subtotal</span>
                              <span className="text-[10px] xs:text-xs font-semibold text-[var(--color-text)]">{formatCurrency(suborder.amount)}</span>
                            </div>
                          </div>
                          ...
                          {suborder.deliveryDetails?.actualTime && (
                            <div className="bg-[var(--color-surface)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                              <div className="flex items-center space-x-1.5 xs:space-x-2 mb-2 xs:mb-3">
                                <Clock className="w-3 h-3 xs:w-4 xs:h-4 text-[var(--color-primary)]" />
                                <span className="text-[10px] xs:text-xs font-medium text-[var(--color-text)]">Delivery Timeline</span>
                              </div>
                          ...
                          {/* Cancellation Reason */}
                          {(suborder.status === 'CANCELLED' || suborder.status === 'FAILED') && suborder.cancellationReason && (
                            <div className="bg-red-500/5 rounded-lg xs:rounded-xl p-3 xs:p-4 border border-red-500/20">
                              <div className="flex items-start space-x-1.5 xs:space-x-2">
                                <AlertCircle className="w-3 h-3 xs:w-4 xs:h-4 text-red-500 mt-0.5" />
                                <div>
                                  <p className="text-[10px] xs:text-xs font-medium text-red-600">Cancellation Reason</p>
                                  <p className="text-[9px] xs:text-xs text-[var(--color-text-muted)] mt-0.5 xs:mt-1">{suborder.cancellationReason}</p>
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          <div className="flex justify-end">
                            <Link
                              href={`/delivery/orders/${delivery._id}/${suborder._id}`}
                              className="inline-flex items-center space-x-1 xs:space-x-2 px-2 xs:px-3 py-1 xs:py-1.5 text-[10px] xs:text-xs text-[var(--color-primary)] hover:underline transition-all"
                            >
          ...
          <div className="mt-6 xs:mt-8 flex justify-center">
            <div className="flex space-x-1 xs:space-x-2">
              <button
                onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                disabled={currentPage === 1}
                className="px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 text-sm xs:text-base bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg text-[var(--color-text)] hover:border-[var(--color-primary)] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                Previous
              </button>
              <span className="px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 text-sm xs:text-base text-[var(--color-text)]">
                Page {currentPage} of {totalPages}
              </span>
              <button
                onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))}
