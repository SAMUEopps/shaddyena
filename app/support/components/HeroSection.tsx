...
// app/shd-pages/components/HeroSection.tsx
'use client';
...
export default function HeroSection() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrent((prev) => (prev + 1) % (advertisements.length || 1));
    }, 5000);
        ...
        <div className="p-6 sm:p-8 lg:p-12 text-white max-w-2xl">
          ...
          {ad.link && (
            <a
              ...
              className="mt-4 inline-block bg-white text-black px-6 py-2 rounded-lg font-semibold hover:bg-primary-light hover:text-white transition"
            >
              Learn More →
            </a>
          )}
  ...
  return (
    <section className="relative bg-gradient-to-br from-secondary via-secondary/95 to-primary/80 text-white overflow-hidden">
      ...
      {/* Decorative Background */}
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        <div className="absolute top-8 left-8 text-7xl sm:text-8xl">🛍️</div>
        <div className="absolute bottom-8 right-8 text-7xl sm:text-8xl">📦</div>
        <div className="absolute top-1/2 left-1/4 text-6xl">✨</div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20 lg:py-28 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          {/*{loading ? (
            ...
            // Show advertisement slides
            <div key={current} className="animate-[fadeIn_0.7s_ease-in-out]">
              {renderAdSlide(currentItem as Advertisement)}
            </div>
          ) : (
          ...
          {/* Buttons - Only show for non-vendors or if no ads */}
          {!isVendor && !hasAdvertisements && (
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Link
                href="/vendor/register"
                className="bg-white text-secondary hover:bg-primary-light hover:text-white px-6 sm:px-8 py-3 rounded-xl font-bold shadow-lg transition-all duration-300 hover:-translate-y-1"
              >
                Become a Vendor →
              </Link>
              <Link
                href="/shops"
                className="bg-white/20 backdrop-blur-md hover:bg-white/30 border border-white/30 px-6 sm:px-8 py-3 rounded-xl font-medium transition-all duration-300"
              >
                View Marketplace
              </Link>
            </div>
          ...
          {isVendor && (
            <div className="mt-8">
              <Link
                ...
                className="bg-white text-secondary hover:bg-primary-light hover:text-white px-6 sm:px-8 py-3 rounded-xl font-bold shadow-lg transition-all duration-300 hover:-translate-y-1 inline-block"
              >
                Go to Dashboard
              </Link>
            </div>
