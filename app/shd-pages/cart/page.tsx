...
import { useApiWithOrg } from '@/shd-lib/lib/useApiWithOrg';
...
export default function Cart() {
  ...
  const router = useRouter();
  const { user } = useAuth();
  const { apiRequest } = useApiWithOrg();
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Use apiRequest from hook - it automatically adds organization ID
      const response = await apiRequest('/api/shd-api/api/checkout', {
        method: 'POST',
        body: JSON.stringify({
          items,
          phoneNumber,
