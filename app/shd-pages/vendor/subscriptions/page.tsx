...
// app/shd-pages/vendor/subscriptions/page.tsx
'use client';
    ...
    <div className="min-h-screen bg-background py-8 sm:py-12">
              ...
              return (
                ...
                >
                  <div className="p-6 sm:p-7">
                    ...
                    {/* Action Button - Opens Payment Modal */}
                    {!isCurrent && sub.isActive && (
                      <button
                        onClick={() => handleSubscribeClick(sub)}
                        disabled={upgrading}
                        className={`mt-6 w-full py-3 rounded-xl font-bold transition-all duration-200 shadow-sm hover:shadow-md active:scale-[0.98] ${
                          isUpgrade
                            ? 'bg-primary hover:bg-accent-dark text-white'
      ...
      {selectedSubscription && (
        <PaymentModal
          ...
          title={`Subscribe to ${selectedSubscription.name}`}
          description={`You will receive an M-Pesa STK Push to complete payment for the ${selectedSubscription.name} plan.`}
          purpose={`${selectedSubscription.tier} subscription`}
        />
      )}
    </div>
  );
