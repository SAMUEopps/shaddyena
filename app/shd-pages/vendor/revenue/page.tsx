...
// app/shd-pages/vendor/revenue/page.tsx - Add status polling for pending withdrawals
'use client';
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    // Poll for withdrawal status updates every 30 seconds if there are pending/processing withdrawals
    const hasPendingWithdrawals = withdrawalHistory.some(w => 
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showMessage('success', 'Withdrawal initiated successfully!');
        setWithdrawAmount('');
        setShowWithdrawModal(false);
        ...
        // Update local stats
        if (vendor && data.newBalance !== undefined) {
          vendor.availableBalance = data.newBalance;
          setRevenueStats(prev => prev ? {
            ...prev,
                ...
                <tbody>
                  {withdrawalHistory.map((withdrawal) => (
                    <tr key={withdrawal._id} className="border-b last:border-0">
                      ...
                      <td className="py-3 text-sm">{withdrawal.method}</td>
                      <td className="py-3">
                        <span className={`text-xs px-2 py-1 rounded-full ${getStatusBadge(withdrawal.status)}`}>
                          {withdrawal.status}
                        ...
                        {withdrawal.errorMessage && (
                          <div className="text-xs text-red-500 mt-1">{withdrawal.errorMessage}</div>
                        )}
                      </td>
                      <td className="py-3 text-sm text-muted">{withdrawal.reference}</td>
                    </tr>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {withdrawMethod === 'MPESA' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                  />
                  <p className="text-xs text-muted mt-1">Enter number in format: 2547XXXXXXXX</p>
                </div>
              )}
              ...
              <p className="text-xs text-muted text-center mt-2">
                {withdrawMethod === 'MPESA' 
                  ? 'Funds will be sent to your M-PESA account immediately' 
                  : 'Bank transfers are processed within 24-48 hours'}
              </p>
            </div>
          </div>
        </div>
