...
interface Stats {
  ...
  statusBreakdown: Array<{ _id: string; count: number; total: number }>;
  typeBreakdown: Array<{ _id: string; count: number; total: number }>;
  categoryBreakdown: Array<{ _id: string; count: number; total: number }>;
  dailyStats: Array<{ _id: string; count: number; total: number; success: number; failed: number; pending: number }>;
  monthlyStats: Array<{ _id: string; count: number; total: number; success: number }>;
  topCustomers: Array<{ phoneNumber: string; count: number; total: number }>;
  recentTransactions: Transaction[];
  summary: {
    totalAmount: number;
    totalPending: number;
...
export default function AdminTransactions() {
  ...
  const handleBulkAction = async (action: string, value?: any) => {
    ...
    const actionMessages: Record<string, string> = {
      ...
      updateStatus: `update status for ${selectedTransactions.length} transactions to "${value}"`,
      updateType: `update type for ${selectedTransactions.length} transactions to "${value}"`,
      updateCategory: `update category for ${selectedTransactions.length} transactions to "${value}"`
    };

    if (!confirm(`Are you sure you want to ${actionMessages[action] || action}?`)) return;
  ...
  const filteredTransactions = transactions.filter(transaction => {
    ...
    if (search) {
      ...
      return transaction.transactionId.toLowerCase().includes(searchLower) ||
             transaction.receiptNumber?.toLowerCase().includes(searchLower) ||
             transaction.phoneNumber?.includes(search) ||
             transaction.accountReference?.toLowerCase().includes(searchLower) ||
             transaction.externalReference?.toLowerCase().includes(searchLower) ||
             transaction.order?.orderNumber?.toLowerCase().includes(searchLower) ||
             transaction.vendor?.businessName?.toLowerCase().includes(searchLower) ||
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl shadow p-4 border-l-4 border-green-500">
            ...
            <div className="text-xs text-muted mt-1">
              Avg: {formatAmount(stats.averageAmount)}
            </div>
          </div>
          <div className="bg-white rounded-xl shadow p-4 border-l-4 border-yellow-500">
            <div className="text-sm text-muted">Pending</div>
            <div className="text-2xl font-bold text-secondary">
              {formatAmount(stats.summary.totalPending)}
            </div>
            <div className="text-xs text-muted mt-1">
              {stats.statusBreakdown.find(s => s._id === 'pending')?.count || 0} transactions
            </div>
          ...
          <div className="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500">
            ...
            <div className="text-2xl font-bold text-secondary">
              {formatAmount(stats.summary.totalProcessing)}
            </div>
            <div className="text-xs text-muted mt-1">
              {stats.statusBreakdown.find(s => s._id === 'processing')?.count || 0} transactions
            </div>
          </div>
          <div className="bg-white rounded-xl shadow p-4 border-l-4 border-red-500">
            <div className="text-sm text-muted">Failed</div>
            <div className="text-2xl font-bold text-secondary">
              {formatAmount(stats.summary.totalFailed)}
            </div>
            <div className="text-xs text-muted mt-1">
              {stats.statusBreakdown.find(s => s._id === 'failed')?.count || 0} transactions
            </div>
          ...
          <div className="bg-white rounded-xl shadow p-4 border-l-4 border-purple-500">
            <div className="text-sm text-muted">Payments</div>
            <div className="text-2xl font-bold text-secondary">
              {stats.typeBreakdown.find(t => t._id === 'payment')?.count || 0}
            </div>
            <div className="text-xs text-muted mt-1">
              Payouts: {stats.typeBreakdown.find(t => t._id === 'payout')?.count || 0}
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-3">
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="border border-accent rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary w-48 sm:w-64"
          />
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
      ...
      {selectedTransactions.length > 0 && (
        <div className="bg-surface p-4 rounded-xl mb-4 flex flex-wrap items-center gap-3">
          ...
          <select
            onChange={(e) => handleBulkAction('updateCategory', e.target.value)}
            className="border rounded-lg px-3 py-2 text-sm bg-white"
            defaultValue=""
          >
            <option value="" disabled>Update Category</option>
            <option value="order">📦 Order</option>
            <option value="membership">👤 Membership</option>
            <option value="savings">💰 Savings</option>
            <option value="investment">📈 Investment</option>
          ...
          <table className="w-full">
            <thead className="bg-surface">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Transaction</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Type</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Category</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Customer/Vendor</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Amount</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-secondary uppercase tracking-wider">Date</th>
            ...
            <tbody className="divide-y divide-accent">
              {filteredTransactions.map((transaction) => (
                <tr key={transaction._id} className="hover:bg-background transition">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getCategoryColor(transaction.category)}`}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {transaction.externalEntityType && (
                      <div className="text-xs text-muted">
                        {transaction.externalEntityType}: {transaction.externalEntityId}
                      </div>
                    )}
                    {!transaction.phoneNumber && !transaction.order && !transaction.vendor && (
                      <div className="text-xs text-muted">
                        {transaction.organizationId?.name || 'N/A'}
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3">
