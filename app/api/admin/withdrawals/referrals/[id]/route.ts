...
export async function PATCH(
  ...
  try {
    ...
    const { status, mpesaTransactionCode, adminNotes } = await req.json();
    ...
    await ReferralWithdrawal.findByIdAndUpdate(withdrawalId, updateData);

    const updatedWithdrawal = await ReferralWithdrawal.findById(withdrawalId)
...
export async function DELETE(
  ...
  try {
    ...
    if (withdrawal.status !== "pending") {
      return NextResponse.json(
        {
          error: "Can only delete pending withdrawal requests",
        },
        { status: 400 }
      );
    }
