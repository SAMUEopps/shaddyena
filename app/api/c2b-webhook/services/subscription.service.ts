...
export class SubscriptionPaymentService extends BasePaymentService {
  ...
  async processPayment(
    ...
    return this.executeTransaction(async (session) => {
      try {
        ...
        if (!subscriptionId || !vendorId) {
          throw new Error('Missing subscription or vendor ID in transaction metadata');
        }
        ...
        // Handle referral commission
        await this.handleReferralCommission(metadata, session);

        logger.info(`Subscription activated for vendor ${vendorId} - Plan: ${subscription.name || subscription._id}`);
        return { success: true, data: { vendorSubscription: vendorSubscription._id } };
      } catch (error) {
        logger.error('Error processing subscription payment:', error);
        return { success: false, message: error instanceof Error ? error.message : 'Unknown error' };
  ...
  private async handleReferralCommission(
    ...
    const userId = metadata.userId || metadata.customerId;
    
    if (!userId) {
      logger.warn('No userId found in transaction metadata, skipping referral commission');
      return;
    }
    ...
    if (user?.referredBy) {
      ...
      if (referrer) {
        // Get transaction amount from metadata or default to 0
        const transactionAmount = metadata.originalAmount || metadata.amount || 0;
        const commissionAmount = transactionAmount * 0.01; // 1% referral commission
        
        referrer.referralSubscriptionEarnings = (referrer.referralSubscriptionEarnings || 0) + commissionAmount;
        referrer.referralEarnings = (referrer.referralEarnings || 0) + commissionAmount;
        referrer.availableBalance = (referrer.availableBalance || 0) + commissionAmount;
        await referrer.save({ session });
        logger.info(`Added ${commissionAmount} subscription referral commission to ${referrer.name || referrer.email || referrer._id}`);
      }
    }
  }
}