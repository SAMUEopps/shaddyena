...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const decoded = verifyToken(token);
    if (!decoded) {
      console.warn(`${LOG} ${requestId} Invalid token`);
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    console.log(`${LOG} ${requestId} Authenticated user ${decoded.userId}`);

    const body = await req.json();
    const { amount, method, phoneNumber, bankDetails } = body;

    console.log(`${LOG} ${requestId} Withdrawal payload`, {
      amount,
      method,
      hasPhone: !!phoneNumber,
      hasBankDetails: !!bankDetails
    ...
    if (typeof amount !== 'number' || amount <= 0) {
      console.warn(`${LOG} ${requestId} Invalid withdrawal amount`, amount);
      return NextResponse.json(
        { error: 'Invalid amount' },
        { status: 400 }
      );
    ...
    // Validate method
    if (!['MPESA', 'BANK'].includes(method)) {
      console.warn(`${LOG} ${requestId} Invalid withdrawal method`, method);
      return NextResponse.json(
        { error: 'Invalid withdrawal method' },
        { status: 400 }
      );
    ...
    if (!vendor) {
      console.warn(`${LOG} ${requestId} Vendor not found`, decoded.userId);
      return NextResponse.json(
        { error: 'Vendor not found' },
        { status: 404 }
      );
    ...
    console.log(`${LOG} ${requestId} Vendor found`, {
      vendorId: vendor._id,
      businessName: vendor.businessName,
      balance: vendor.availableBalance
    });
    ...
    if (amount > vendor.availableBalance) {
      console.warn(`${LOG} ${requestId} Insufficient balance`, {
        requested: amount,
        available: vendor.availableBalance
      });
      return NextResponse.json(
    ...
    console.log(`${LOG} ${requestId} Creating withdrawal`, {
      reference,
      amount
    });

    // FIXED: Create withdrawal with correct fields for your model
    ...
    // Only add phoneNumber for MPESA
    if (method === 'MPESA' && phoneNumber) {
      withdrawalData.phoneNumber = phoneNumber;
    }

    // Only add bankDetails for BANK
    ...
    console.log(`${LOG} ${requestId} Withdrawal created`, withdrawal._id);

    /**
     * MPESA B2C FLOW
     */
    if (method === 'MPESA') {
      // Use provided phone or get from vendor
      const mpesaPhone = phoneNumber || vendor.payoutDetails?.mpesaNumber || vendor.phoneNumber;
      
      if (!mpesaPhone) {
        console.warn(`${LOG} ${requestId} Missing MPESA phone`);
        withdrawal.status = 'failed';
        withdrawal.errorMessage = 'Phone number required for MPESA';
        await withdrawal.save();
      ...
      try {
        ...
        console.log(`${LOG} ${requestId} Initiating MPESA B2C`, {
          phone: formattedPhone,
          amount,
          reference
        });
        ...
        console.log(`${LOG} ${requestId} MPESA B2C response`, result);

        // Update withdrawal with transaction ID
        withdrawal.transactionId = result.ConversationID || result.conversationID;
        withdrawal.status = 'processing'; // lowercase - matches your model
        ...
        console.log(`${LOG} ${requestId} Withdrawal processing`, {
          transactionId: withdrawal.transactionId,
          newBalance: vendor.availableBalance
        });
        ...
        console.error(`${LOG} ${requestId} MPESA B2C failed`, {
          message: error.message,
          stack: error.stack
        });
     ...
     * BANK WITHDRAWAL FLOW
     */
    console.log(`${LOG} ${requestId} Processing BANK withdrawal`);
    withdrawal.status = 'processing';
    withdrawal.metadata = {
      ...withdrawal.metadata,
      bankProcessedAt: new Date().toISOString()
    };
    ...
    console.log(`${LOG} ${requestId} Bank withdrawal queued`, {
      withdrawalId: withdrawal._id,
      newBalance: vendor.availableBalance
    });
    ...
    console.error(`${LOG} ${requestId} Fatal withdrawal error`, {
      message: error.message,
      stack: error.stack
    });
