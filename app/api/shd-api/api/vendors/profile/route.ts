...
export async function GET(req: NextRequest) {
  try {
    ...
    // Ensure all revenue fields exist with defaults
    const vendorWithDefaults = {
      ...
      totalEarned: vendor.totalEarned || vendor.totalRevenue || 0,
      pendingPayout: vendor.pendingPayout || vendor.pendingBalance || 0,
    };

    return NextResponse.json({ vendor: vendorWithDefaults });
  } catch (error) {
    console.error('Fetch vendor profile error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch vendor profile' },
