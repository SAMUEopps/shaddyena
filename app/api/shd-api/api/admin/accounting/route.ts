...
// Import actual types from the Transaction model
import type { TransactionType, TransactionCategory, TransactionStatus } from '@/shd-models/models/Transaction';

type FinancialDirection = 'payment' | 'deposit' | 'payout' | 'refund';
type BusinessCategory = TransactionCategory;
...
function calculateTotals(transactions: TransactionLike[]) {
  const totals = {
    ...
    byCategory: {
      ...
      customer_payment: 0,
      other: 0,
    } satisfies Record<BusinessCategory, number>,
  };

  transactions.forEach((t) => {
    const amount = t.amount ?? 0;

    totals.byCategory[t.category] += amount;
    ...
    if (t.category === 'order' && t.status === 'pending') {
      totals.pendingSettlements += amount;
    }
  });
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Calculate totals
    const totals = calculateTotals(transactions as TransactionLike[]);
    ...
    // Get recent transactions (no populate - Transaction doesn't have userId/vendorId refs)
    const recentTransactions = await Transaction.find({
      ...dateFilter,
      status: { $in: ['success', 'pending', 'failed'] }
    })
    ...
    // Get revenue breakdown by transaction category (not type)
...
export async function POST(req: NextRequest) {
  try {
    await connectToDatabase();
    const body = await req.json();
    const { startDate, endDate, type, category } = body;
    ...
    // Format for CSV - extract user/vendor info from metadata since no refs exist
    const csvData = transactions.map(t => {
      ...
      return {
        ...
        'User ID': meta.userId || meta.customerId || 'N/A',
        'Vendor ID': meta.vendorId || 'N/A',
        'Purpose': t.purpose || 'N/A',
        'External Reference': t.externalReference || 'N/A'
      };
    });
