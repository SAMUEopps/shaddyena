...
// GET - Fetch active budget for the test user
export async function GET(req: NextRequest) {
  try {
    await connectToDatabase();

    console.log('Fetching budget for test user:', TEST_USER_ID);
...
// POST - Create a new budget for the test user
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    ...
    console.log('Creating budget for test user:', TEST_USER_ID);
    ...
    // Close existing active budget
    if (existingBudget) {
      existingBudget.status = 'closed';
      await existingBudget.save();

      console.log(
        'Closed existing budget:',
        existingBudget._id
      );
    }
