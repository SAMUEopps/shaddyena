...
import { ArrowLeft, CreditCard, Smartphone, CheckCircle, AlertCircle, Crown, Sparkles, Shield, Clock, TrendingUp, Rocket, Star, Zap, Gift, Award, Loader2 } from 'lucide-react';
...
export default function SubscriptionPayment() {
  ...
  const fetchSubscriptionPlans = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Auto-select the first plan or popular plan
        const popularPlan = data.find((p: SubscriptionPlan) => p.popular);
        if (popularPlan) {
  ...
  const selectedPlanData = subscriptionPlans.find(plan => plan._id === selectedPlan);
      ...
      <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 py-4 xs:py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 xs:gap-7 sm:gap-8">
          ...
          <div className="lg:col-span-2 space-y-5 xs:space-y-6 sm:space-y-8">
            ...
            <div className="grid grid-cols-1 gap-3 xs:gap-4">
              {subscriptionPlans.map((plan) => {
                const IconComponent = iconMap[plan.icon || 'Star'] || Star;
                return (
                  <div
                    key={plan._id}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="mb-3">
                          ...
                          <span className="text-[9px] xs:text-[10px] sm:text-xs text-[var(--color-text-muted)]">/{periodLabels[plan.period]}</span>
                        </div>
                        <ul className="space-y-1.5 xs:space-y-2">
                          ...
                          {plan.features.length > 5 && (
                            <li className="text-[9px] xs:text-[10px] sm:text-xs text-[var(--color-primary)]">
            ...
            <div className="bg-[var(--color-surface)] rounded-xl xs:rounded-2xl border border-[var(--color-border)] p-4 xs:p-5 sm:p-6 sticky top-6">
              ...
              {!showPaymentForm ? (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-alt)] text-white py-2.5 xs:py-3 px-4 rounded-lg xs:rounded-xl hover:opacity-90 transition-all duration-300 font-medium text-[10px] xs:text-xs sm:text-sm shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Pay {selectedPlanData ? formatPrice(selectedPlanData.price) : 'Select a plan'} Now
                </button>
              ) : (
                <div className="bg-[var(--color-background)] rounded-lg xs:rounded-xl p-3 xs:p-4 border border-[var(--color-border)]">
                  <label className="block text-[9px] xs:text-[10px] sm:text-xs font-medium text-[var(--color-text)] mb-1.5 xs:mb-2">
