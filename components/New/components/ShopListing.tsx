  ...
  return (
    <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="relative mb-6 sm:mb-8">
        <div className="absolute inset-0 flex items-center justify-center opacity-5">
          <Store className="w-24 h-24 sm:w-32 text-[var(--color-primary)]" />
        </div>
      ...
      <div className="relative mb-6 sm:mb-8">
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
          ...
          {/* View Toggle */}
          <div className="hidden sm:flex items-center gap-2 p-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg transition-all ${
                viewMode === 'grid' 
        ...
        {/* Filters */}
        <div className={`mt-3 sm:mt-4 flex flex-wrap gap-2 sm:gap-3 ${!showFilters && 'hidden lg:flex'}`}>
          <select
            value={selectedCity}
            onChange={(e) => {
              setSelectedCity(e.target.value);
          ...
          {(searchQuery || selectedCity || selectedType) && (
            <button
              onClick={clearFilters}
              className="px-3 sm:px-4 py-2 text-[var(--color-text-muted)] hover:text-[var(--color-primary)] flex items-center gap-1.5 sm:gap-2 transition-all text-sm"
            >
      ...
      {/* Shops Grid/List - REFACTORED FOR 2 COLUMNS ON MOBILE */}
      {loading ? (
        <div className={`grid gap-2 xs:gap-3 sm:gap-4 lg:gap-6 ${
          ...
          {[1,2,3,4,5,6,7,8].map(i => (
            <div key={i} className="bg-[var(--color-surface)] rounded-xl sm:rounded-2xl border border-[var(--color-border)] overflow-hidden animate-pulse">
              <div className="h-20 xs:h-24 sm:h-32 lg:h-40 bg-[var(--color-border)]"></div>
              <div className="p-2 xs:p-3 sm:p-4 lg:p-5">
                <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                  <div className="w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-[var(--color-border)]"></div>
                  <div className="flex-1">
                    <div className="h-3 sm:h-4 bg-[var(--color-border)] rounded w-3/4 mb-1 sm:mb-2"></div>
                    <div className="h-2 sm:h-3 bg-[var(--color-border)] rounded w-1/2"></div>
                  </div>
                </div>
                <div className="space-y-1 sm:space-y-2">
                  <div className="h-2 sm:h-3 bg-[var(--color-border)] rounded w-full"></div>
                  <div className="h-2 sm:h-3 bg-[var(--color-border)] rounded w-5/6 hidden sm:block"></div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="text-center py-12 sm:py-16">
          <Store className="w-12 h-12 sm:w-16 text-[var(--color-text-muted)] mx-auto mb-4 opacity-50" />
          <h3 className="text-lg sm:text-xl font-semibold text-[var(--color-text)] mb-2">No shops found</h3>
            ...
            {shops.map((shop) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-1.5 right-1.5 xs:top-2 xs:right-2 sm:top-3 sm:right-3 z-10 p-1.5 xs:p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-md hover:scale-110 transition-transform"
                >
                  <Heart 
                ...
                <div className={`relative overflow-hidden ${viewMode === 'list' ? 'md:w-48 h-32 sm:h-40' : 'h-16 xs:h-20 sm:h-24 lg:h-32'}`}>
                  {shop.banner ? (
                    <Image
                      src={shop.banner}
                      alt={shop.businessName}
                  ...
                  {shop.isVerified && (
                    <div className="absolute top-1.5 left-1.5 xs:top-2 xs:left-2 sm:top-3 sm:left-3 bg-white/90 backdrop-blur-sm text-[var(--color-primary)] px-1.5 xs:px-2 py-0.5 xs:py-1 rounded-full text-[8px] xs:text-[10px] sm:text-xs font-bold flex items-center gap-0.5 xs:gap-1 shadow-md">
                ...
                {/* Shop Info - Responsive padding */}
                <div className={`relative px-2 xs:px-3 sm:px-4 lg:px-5 pb-2 xs:pb-3 sm:pb-4 lg:pb-5 ${viewMode === 'list' ? 'md:flex-1' : ''}`}>
                  {/* Logo - Responsive sizing */}
                  <div className={`relative ${viewMode === 'list' ? '-mt-4 sm:-mt-6' : '-mt-2 xs:-mt-3 sm:-mt-4 lg:-mt-8'} mb-1.5 xs:mb-2 sm:mb-3`}>
                    <div className={`relative rounded-full overflow-hidden border-2 xs:border-2 sm:border-4 border-[var(--color-surface)] shadow-md sm:shadow-xl bg-[var(--color-surface)] ${
                      viewMode === 'list' ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 lg:w-14 lg:h-14'
                    }`}>
                      {shop.logo ? (
                        <Image
                          src={shop.logo}
                          ...
                          fill
                          className="object-cover"
                          sizes={viewMode === 'list' ? '48px' : '(max-width: 640px) 24px, (max-width: 1024px) 40px, 56px'}
                        />
                      ) : (
                        <div className="w-full h-full bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-alt)] flex items-center justify-center">
                  ...
                  {/* Shop Details - Responsive text */}
                  <div className="mb-1 xs:mb-2 sm:mb-3">
                    <h3 className="text-[10px] xs:text-xs sm:text-sm lg:text-lg font-bold text-[var(--color-primary-alt)] mb-0.5 xs:mb-1 line-clamp-1">
                      {shop.businessName}
                    </h3>
                    ...
                    {shop.rating && shop.rating.count > 0 && (
                      <div className="flex items-center gap-0.5 xs:gap-1 bg-[var(--color-primary-soft)]/20 px-1 xs:px-1.5 sm:px-2 py-0.5 rounded-lg w-fit">
                  ...
                  <p className="hidden xs:block text-[8px] xs:text-[10px] sm:text-xs text-[var(--color-text-muted)] mb-1.5 xs:mb-2 sm:mb-3 line-clamp-2">
                    {shop.description || 'No description available'}
                  </p>

                  {/* Location - Responsive text */}
                  <div className="flex items-center gap-0.5 xs:gap-1 sm:gap-1.5 text-[8px] xs:text-[10px] sm:text-xs text-[var(--color-text-muted)] mb-1.5 xs:mb-2 sm:mb-3">
                  ...
                  <div className="flex items-center gap-1 xs:gap-1.5 sm:gap-2 text-[8px] xs:text-[10px] sm:text-xs text-[var(--color-text-muted)] mb-2 xs:mb-3">
                    <Phone className="w-2 h-2 xs:w-2.5 xs:h-2.5 sm:w-3 sm:h-3 text-[var(--color-primary)] flex-shrink-0" />
                    <span className="line-clamp-1">{shop.contact.phone}</span>
                  </div>

                  {/* Visit Button - Responsive */}
                  <button className="w-full bg-[var(--color-primary-alt)] text-white py-1 xs:py-1.5 sm:py-2 rounded-md xs:rounded-lg text-[8px] xs:text-[10px] sm:text-xs font-medium hover:bg-[var(--color-primary-hover)] transition-all duration-300 hover:shadow-md hover:shadow-[var(--color-primary)]/20">
                    <span className="hidden xs:inline">Visit Shop</span>
            ...
            <div className="flex items-center justify-center gap-1 sm:gap-2 mt-8 sm:mt-12">
              ...
              {[...Array(pagination.totalPages)].map((_, i) => {
                ...
                if (
                  ...
                  return <span key={pageNum} className="text-[var(--color-text-muted)] px-1">...</span>;
                }
                return null;
              })}
