...
const Topbar = ({ isDarkMode }: TopbarProps) => {
  ...
  return (
    <div className="relative z-[60] bg-[var(--color-background-soft)] border-b border-[var(--color-border)] text-[var(--color-text-muted)] text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/*<div className="flex items-center justify-between h-10">
          
          {/* Left side - Shipping info with animated truck *
          <div className="flex items-center space-x-6 relative z-[61]">
            <div className="hidden sm:flex items-center space-x-2 group">
              <div className="relative">
                <Truck className="w-4 h-4 text-[var(--color-primary)] group-hover:animate-bounce-subtle" />
                <span className="absolute -top-1 -right-1 w-2 h-2 bg-[var(--color-primary)] rounded-full animate-ping opacity-75"></span>
          ...
          {/* Right side - Currency & Language selectors *
          <div className="flex items-center space-x-4 relative z-[62]">
            {/* Currency Selector *
            <div className="relative z-[110]">
              <button
                ...
                className="relative z-[111] flex items-center space-x-1 px-2 py-1 rounded-lg hover:bg-[var(--color-primary-soft)] hover:text-[var(--color-primary)] transition-all duration-200 group"
              >
                <DollarSign className="w-4 h-4" />
                <span>{selectedCurrency}</span>
                <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${showCurrency ? 'rotate-180' : ''}`} />
              ...
              {/* Currency Dropdown *
              {showCurrency && (
                <div className="absolute top-full right-0 mt-1 w-24 bg-[var(--color-surface)] rounded-lg shadow-2xl border border-[var(--color-border)] overflow-hidden z-[120] animate-in fade-in slide-in-from-top-2 duration-200">
                  {currencies.map((currency) => (
                    <button
                      key={currency}
                      onClick={() => {
  ...
  {/* LEFT side - Currency & Language selectors (was right) */}
  <div className="flex items-center space-x-4 relative z-[62]">
  ...
  {/* RIGHT side - Shipping info (was left) */}
  <div className="flex items-center space-x-6 relative z-[61]">
    <div className="hidden sm:flex items-center space-x-2 group">
      <div className="relative">
        <Truck className="w-4 h-4 text-[var(--color-primary)] group-hover:animate-bounce-subtle" />
