...
// components/tabs/OrdersTab/AdminOrdersTab.tsx
'use client';
...
export default function AdminOrdersTab() {
  ...
  const statCards = [
    { label: 'Total Orders', value: stats.total, icon: Package, color: 'from-blue-500 to-cyan-500', bg: 'bg-blue-500/10', textColor: 'text-blue-600' },
    ...
    { label: 'Processing', value: stats.processing, icon: Settings, color: 'from-purple-500 to-pink-500', bg: 'bg-purple-500/10', textColor: 'text-purple-600' },
    ...
    { label: 'Paid', value: stats.paid, icon: CreditCard, color: 'from-teal-500 to-green-500', bg: 'bg-teal-500/10', textColor: 'text-teal-600' },
  ];

  const getStatusBadge = (status: string) => {
    const config: Record<string, { icon: any, color: string, bg: string, label: string }> = {
      PENDING: { icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-100', label: 'Pending' },
      PROCESSING: { icon: Settings, color: 'text-purple-600', bg: 'bg-purple-100', label: 'Processing' },
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[var(--color-primary)]/10 via-[var(--color-primary-soft)]/5 to-transparent rounded-2xl p-6 md:p-8 mb-8">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-[var(--color-primary)]/10 rounded-full mb-4">
            ...
            <span className="text-sm text-[var(--color-primary)] font-medium">Admin Dashboard</span>
          </div>
      ...
      {error ? (
        ...
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--color-text)] mb-2">No orders found</h3>
          <p className="text-[var(--color-text-muted)]">
            No orders match your current filters.
          </p>
        </div>
                    ...
                    return (
                      <tr key={order._id} className="hover:bg-[var(--color-background-soft)] transition-colors group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-xs text-[var(--color-text-muted)]">
                            Fee: {OrderService.formatCurrency(order.platformFee)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => router.push(`/orders/${order._id}`)}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--color-background-soft)] text-[var(--color-text-muted)] rounded-lg text-xs font-medium hover:text-[var(--color-primary)] transition-all duration-300"
                            >
