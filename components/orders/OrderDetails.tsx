...
export default function OrderDetails({ orderId }: OrderDetailsProps) {
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      // If vendor is viewing and we have suborderId, find their specific suborder
      if (user.role === 'vendor' && vendorView && data.order) {
        ...
        if (foundSuborder) {
          // Get the original items for this vendor from the main order
    ...
    try {
      ...
      pdfContainer.innerHTML = `
        <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #333; padding-bottom: 20px;">
          ...
          <p style="margin: 5px 0; color: #666;">Order ID: ${order.orderId}</p>
          <p style="margin: 5px 0; color: #666;">Date: ${formatDate(order.createdAt)}</p>
          ${user?.role === 'vendor' && vendorSuborder ? `<p style="margin: 5px 0; color: #666;">Vendor Suborder</p>` : ''}
        </div>
      ...
      pdf.addImage(imgData, 'PNG', imgX, imgY, imgWidth * ratio, imgHeight * ratio);
      pdf.save(`order-${order.orderId}${user?.role === 'vendor' ? '-vendor' : ''}.pdf`);
    } catch (error) {
      console.error('Error generating PDF:', error);
      setError('Failed to generate PDF. Please try again.');
    } finally {
  ...
  // Get vendor-specific summary for PDF
  const getPDFPaymentSummary = () => {
    if (!order) return '';
    ...
    if (user?.role === 'vendor' && vendorSuborder) {
      return `
        <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
          <span><strong>Subtotal:</strong></span>
          <span>${formatCurrency(vendorSuborder.suborder.amount, order.currency)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
          <span><strong>Commission (${((vendorSuborder.suborder.commission / vendorSuborder.suborder.amount) * 100).toFixed(1)}%):</strong></span>
          <span>${formatCurrency(vendorSuborder.suborder.commission, order.currency)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; font-size: 18px; font-weight: bold; margin-top: 15px; padding-top: 15px; border-top: 1px solid #ddd;">
          <span>Net Amount:</span>
          <span>${formatCurrency(vendorSuborder.suborder.netAmount, order.currency)}</span>
        </div>
      `;
    }
      ...
      return (
        <>
          <div className="flex justify-between">
            <span className="text-sm text-gray-600">Subtotal</span>
            <span className="text-sm text-gray-900">
              {formatCurrency(vendorSuborder.suborder.amount, order.currency)}
            </span>
          </div>
          <div className="flex justify-between">
            {/*<span className="text-sm text-gray-600">
              Commission ({(vendorSuborder.suborder.commission / vendorSuborder.suborder.amount * 100).toFixed(1)}%)
            </span>*
            <span className="text-sm text-gray-600">
              Commission
            </span>
            <span className="text-sm text-gray-900">
              {formatCurrency(vendorSuborder.suborder.commission, order.currency)}
            </span>
          </div>
          <div className="border-t border-gray-200 pt-3 flex justify-between">
            <span className="text-base font-semibold text-gray-900">Net Amount</span>
            <span className="text-base font-semibold text-gray-900">
              {formatCurrency(vendorSuborder.suborder.netAmount, order.currency)}
            </span>
          </div>
        </>
      );
  ...
  // Get current vendor suborder status if viewing as vendor
  const getCurrentStatus = () => {
    if (user?.role === 'vendor' && vendorSuborder) {
      return vendorSuborder.suborder.status;
    }
    ...
    try {
      const response = await fetch('/api/orders/update-status', {
        ...
        body: JSON.stringify({
          orderId: order._id,
          status: newStatus,
          suborderId: vendorSuborder?.suborder._id
        }),
      });
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="mt-4 lg:mt-0 flex flex-col items-end gap-2">
              <div className="flex items-center gap-2">
                <span className={`px-4 py-2 rounded-full text-sm font-semibold border ${getStatusColor(currentStatus)}`}>
                  {isVendorViewingOrder ? `Your Status: ${currentStatus}` : currentStatus}
                </span>
                {isVendorViewingOrder && currentStatus === 'PROCESSING' && (
                  <button
                ...
                {isVendorViewingOrder && currentStatus === 'SHIPPED' && (
                  <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div key={index} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <h3 className="font-medium text-gray-900">Vendor Order #{index + 1}</h3>
                            <p className="text-sm text-gray-600">Vendor ID: {suborder.vendorId}</p>
                            <p className="text-xs text-gray-500">{suborder.items.length} item{suborder.items.length !== 1 ? 's' : ''}</p>
                          </div>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(suborder.status)}`}>
                            {suborder.status}
                          </span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-6">
                <div className="space-y-4">
                  ...
                  {isVendorViewingOrder && vendorSuborder && (
                    <div>
                      <h3 className="text-sm font-medium text-gray-700">Your Net Amount</h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900">
                  {isVendorViewingOrder ? 'Vendor Payment Summary' : 'Payment Summary'}
                </h2>
              </div>
              <div className="p-6">
                <div className="space-y-3">
