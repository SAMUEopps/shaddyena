...
// app/orders/[id]/page.tsx (or OrderDetails main component)
'use client';
...
export default function OrderDetails({ orderId }: OrderDetailsProps) {
  ...
  const handleStatusUpdate = async (newStatus: string, isMainOrder: boolean = false, suborderId?: string) => {
    ...
    try {
      ...
      if (selectedRiderId) requestBody.riderId = selectedRiderId;
      if (deliveryFee > 0) requestBody.deliveryFee = deliveryFee;
  ...
  if (error) return <ErrorState error={error} onRetry={fetchOrder} />;
  if (!order) return <OrderNotFound isRider={isRider} />;
