...
const baseNavItems = [
  ...
  { id: 'seller-requests', label: 'Seller Requests', icon: '👥' },
  { id: 'order-payments', label: 'Order Payments', icon: '💳' },
  { id: 'delivery-earnings', label: 'Delivery Earnings', icon: '💰' },
  { id: 'order-earnings', label: 'Order Earnings', icon: '💳' },
  { id: 'users', label: 'Users', icon: '👤' },
  { id: 'refferal-earnings', label: 'Subs Earnings', icon: '🎁' },
  { id: 'payments', label: 'Payments', icon: '💳' },
...
export default function Home() {
  ...
  const searchParams = useSearchParams();
  const initialTab = searchParams.get('tab'); // Get initial tab from URL params
  const [activeTab, setActiveTab] = useState(
    initialTab && baseNavItems.some(i => i.id === initialTab)
      ? initialTab
      : 'home'
  );
  const { totalItems: cartItemsCount } = useCart(); // Cart context
  const { wishlistItems } = useWishlist(); // Wishlist context
  const currentUser = user || null; // Current user or null
  const [mobileSearchOpen, setMobileSearchOpen] = useState(false); // Mobile search state
  const [isVendorModalOpen, setIsVendorModalOpen] = useState(false); // Become vendor modal state
  const [showVendorSuccess, setShowVendorSuccess] = useState(false); // Vendor success message state
  ...
  const navItems = useMemo(() => {
    ...
    // Customer restrictions - hide admin/management tabs
    if (user.role === 'customer') {
      filteredItems = filteredItems.filter(
        item =>
          !['dashboard', 'delivery', 'delivery-earnings', 'seller-requests', 'users', 'subscription', 'order-payments'].includes(item.id)
    ...
    // Vendor restrictions - hide admin-only tabs
    if (user.role === 'vendor') {
      filteredItems = filteredItems.filter(
        item => !['seller-requests', 'users', 'delivery', 'delivery-earnings'].includes(item.id)
      );
    ...
    // Delivery role - only show delivery-related tabs
    if (user.role === 'delivery') {
      filteredItems = filteredItems.filter(item =>
        ['home', 'delivery', 'orders', 'refferals', 'order-earnings','delivery-earnings', 'refferal-earnings'].includes(item.id)
      );
    ...
    // Admin sees everything except seller requests and users tabs for non-admins
    if (user.role !== 'admin') {
      filteredItems = filteredItems.filter(
        item => !['seller-requests', 'users'].includes(item.id)
      );
  ...
  // Handle vendor application success
  const handleVendorSuccess = () => {
    setShowVendorSuccess(true);
    setTimeout(() => setShowVendorSuccess(false), 5000);
  };
      ...
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0">
          <div className="flex flex-col flex-grow bg-[#182155] pt-5 pb-4 overflow-y-auto">
            ...
            {/* Support Card and Logout Section */}
            <div className="px-4 mt-auto space-y-4">
              <div className="bg-[#0f183f] text-white p-4 rounded-lg">
                <p className="text-sm font-medium">Need help?</p>
                <p className="text-xs mt-1">Contact our support team 24/7</p>
          ...
          <header className="bg-white shadow-sm sticky top-0 z-10">
            <div className="px-4 sm:px-6 lg:px-8">
              <div className="flex justify-between items-center h-16">
                ...
                {/* Left section: Hamburger menu and search (mobile/tablet) */}
                <div className="flex items-center flex-1 lg:flex-none">
                  {/* Hamburger menu - mobile only */}
                  <button
                    className="lg:hidden rounded-md p-2 text-gray-400 hover:text-gray-500 mr-2"
                ...
                <div className="flex items-center space-x-1 sm:space-x-2">     
                  <div className="flex items-center space-x-1 sm:space-x-3">
                    ...
                    {/* Add product button for vendors (hidden on mobile) */}
                    {currentUser?.role === 'vendor' && (
                      <Link
                        href="/vendor/products/add"
                        className="hidden md:flex bg-gray-100 p-2 rounded-full text-gray-400 hover:text-gray-500"
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="relative cursor-pointer ml-1 sm:ml-0"
                      >
                        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[#bf2c7e] rounded-full grid place-items-center text-white font-semibold text-sm sm:text-base">
                          {currentUser?.firstName?.charAt(0)}{currentUser?.lastName?.charAt(0)}
                        </div>
                      ...
                      {userMenuOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50 border border-gray-200">
                          ...
                          {/* Earnings & Withdrawals link for vendors */}
                          {currentUser?.role === 'vendor' && (
                            <Link 
                          ...
                          {/* Withdrawal Requests link for admins */}
                          ...
                          {/* Become a Vendor button for customers */}
                          {currentUser?.role === 'customer' && (
                            <button
                              onClick={() => {
                                setIsVendorModalOpen(true);
              ...
              {/* Mobile search bar (appears when mobileSearchOpen is true) */}
              {mobileSearchOpen && (
                <div className="sm:hidden pb-3">
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
